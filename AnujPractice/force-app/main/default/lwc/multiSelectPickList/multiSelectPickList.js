...
export default class MultiSelectPickList extends LightningElement {
    ...
    connectedCallback() {
        ...
        this.value = value;
        this.values = values;
        this.optionData = JSON.parse(JSON.stringify(optionData));
        this.newOptions = JSON.parse(JSON.stringify(optionData));
    }

    filterOptions(event) {
        this.searchString = event.target.value;

        if (!this.searchString) this.showDropdown = false

        this.noResultMessage = ''

        this.optionData = this.newOptions.filter(
            (item) => item.label.toLowerCase().trim().startsWith(this.searchString.toLowerCase().trim())
        );
        ...
        // this.optionData = this.optionData.filter(
        //     (item) => item.label.toLowerCase().trim().startsWith(this.searchString.toLowerCase().trim())
        //   );
        //   this.showOptions()
    ...
    handleBlur() {
        var previousLabel;
        var count = 0;
        this.optionData = this.newOptions
